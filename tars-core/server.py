...
def react_to_weather(new_weather, old_weather):
    ...
    if new_weather in ('rainy', 'pouring', 'lightning', 'lightning-rainy'):
        ...
        why = f'Rainy weather: closed dining blinds + DJ rainy mood [{"→".join(why_parts)}]'
        ...
        why = f'Sun returned: opened blinds [{"→".join(why_parts)}]'
...
def is_cooper_here():
    """Canonical: read input_boolean.cooper_here from HA (synced from
    Google Calendar Phase 4 events every 5 min by cooper_presence_sync.py).
    Falls back to hardcoded COOPER_SCHED only if HA unreachable.
    cooper_override (manual /cooper/override API) still wins.
    """
    if cooper_override is not None: return cooper_override
    # Primary source of truth: HA input_boolean.cooper_here
    try:
        s = ha_get('/states/input_boolean.cooper_here')
        if s and s.get('state') in ('on', 'off'):
            return s['state'] == 'on'
    except Exception as e:
        logger.warning(f'is_cooper_here: HA lookup failed, falling back to schedule: {e}')
    # Fallback: hardcoded schedule (legacy; only used if HA unreachable)
    now = datetime.now()
...
def on_mode_change(old, new):
    ...
    # v5.1: Thermostat adjusts with mode transitions (Tier 2)
    _thermostat_for_mode(new)
...
# ─── Thermostat Intelligence (Tier 2-5, added 2026-04-29) ─────────────────────────────────────

THERMO_ENTITY = 'climate.home'

# Tier 2: Mode-based temperature schedule
THERMO_MODE_MAP = {
    'morning':      {'temp': 70, 'preset': 'none'},
    'working':      {'temp': 70, 'preset': 'none'},
    'evening':      {'temp': 70, 'preset': 'none'},
    'night':        {'temp': 68, 'preset': 'none'},   # sleep — 2° cooler
    'away':         {'temp': None, 'preset': 'eco'},   # eco mode when away
    'cooper_day':   {'temp': 72, 'preset': 'none'},    # kids prefer warmer
    'cooper_night': {'temp': 70, 'preset': 'none'},    # Cooper bedtime
}

def _thermostat_for_mode(mode):
    """Adjust thermostat based on mode transition (Tier 2)."""
    cfg = THERMO_MODE_MAP.get(mode)
    if not cfg: return
    try:
        if cfg.get('preset') == 'eco':
            ha_post('/services/climate/set_preset_mode',
                    {'entity_id': THERMO_ENTITY, 'preset_mode': 'eco'})
            logger.info(f'🌡️ Thermostat → eco ({mode})')
        else:
            # Clear eco preset first if set
            ha_post('/services/climate/set_preset_mode',
                    {'entity_id': THERMO_ENTITY, 'preset_mode': 'none'})
            if cfg.get('temp'):
                ha_post('/services/climate/set_temperature',
                        {'entity_id': THERMO_ENTITY, 'temperature': cfg['temp']})
                logger.info(f'🌡️ Thermostat → {cfg["temp"]}°F ({mode})')
    except Exception as e:
        logger.error(f'thermostat_for_mode({mode}): {e}')


def check_thermostat_anomaly():
    """Tier 4+5: Check for HVAC anomalies. Called from mode_updater_thread."""
    try:
        s = ha_get(f'/states/{THERMO_ENTITY}')
        if not s: return
        ...
        # Tier 5: Door open while HVAC actively running
        if action in ('cooling', 'heating'):
            door = ha_get('/states/binary_sensor.front_door_lock_door')
            if door and door.get('state') == 'on':  # door open
                notify_mobile(
                    title='🚪❄️ Door open + HVAC running',
                    message=f'Front door is open while HVAC is {action}. Energy waste!',
                    priority='active',
                )
                log_decision('thermostat_alert', [
                    {'action': 'door_open_hvac_alert', 'reason': f'HVAC {action}, door open'}
                ], source='thermostat_check',
                why=f'Door open while HVAC is {action} — sent alert')

        # Tier 4: Temperature anomaly — indoor temp drifting fast
        # (Uses SQLite event history for temp change rate)
        if current and target:
            diff = abs(current - target)
            if diff > 5 and action == 'idle':
                # HVAC is idle but temp is 5°+ off target — something wrong
                _anom_append({
                    'type': 'thermostat_drift',
                    'entity': THERMO_ENTITY,
                    'message': f'Temp {current}°F is {diff:.0f}° off target {target}°F but HVAC idle',
                    'severity': 'medium' if diff < 8 else 'high',
                    'time': datetime.now().isoformat(),
                })
    except Exception as e:
        logger.error(f'check_thermostat_anomaly: {e}')


def check_weather_reactive_thermostat():
    """Tier 3: Weather-reactive pre-conditioning.
    If tomorrow's forecast high > 90°F and it's 10pm-6am, pre-cool to 68°F."""
    try:
        w = ha_get('/states/weather.forecast_home')
        if not w: return
        temp_now = w.get('attributes', {}).get('temperature')
        # Check forecast via HA's weather entity forecast attribute
        forecast = w.get('attributes', {}).get('forecast', [])
        if not forecast: return
        # Look at tomorrow's forecast
        tomorrow_high = None
        for f in forecast[:3]:  # next 3 periods
            t = f.get('temperature')
            if t and (tomorrow_high is None or t > tomorrow_high):
                tomorrow_high = t
        if tomorrow_high is None: return
        h = datetime.now().hour
        # Pre-cool overnight if tomorrow is hot
        if tomorrow_high > 90 and (h >= 22 or h < 6):
            s = ha_get(f'/states/{THERMO_ENTITY}')
            current_target = (s or {}).get('attributes', {}).get('temperature', 70)
            if current_target > 68:  # don't pre-cool if already cool
                ha_post('/services/climate/set_temperature',
                        {'entity_id': THERMO_ENTITY, 'temperature': 68})
                log_decision('weather_precool', [
                    {'action': 'precool_68', 'reason': f'Tomorrow forecast {tomorrow_high}°F'}
                ], source='weather_reactive',
                why=f'Pre-cooling to 68°F overnight — tomorrow\'s high is {tomorrow_high}°F')
                logger.info(f'🌡️ Pre-cool to 68°F — tomorrow {tomorrow_high}°F')
    except Exception as e:
        logger.error(f'weather_reactive_thermostat: {e}')
...
def intel_handle_event(ev):
    ...
    # Sun below horizon
    elif eid == 'sun.sun' and new == 'below_horizon':  # lighting disabled — HA owns sunset scenes
        pass  # hue/ambient/sunset removed 2026-06-03; HA automations own lighting
    # Vacuum done
...
def arrive_sequence():
    ...
    # v5.1: thermostat → comfort mode on arrival
    try:
        comfort_temp = 72 if is_cooper_here() else 70
        ha_post('/services/climate/set_preset_mode', {'entity_id': 'climate.home', 'preset_mode': 'none'})
        ha_post('/services/climate/set_temperature', {'entity_id': 'climate.home', 'temperature': comfort_temp})
    ...
    if is_silent_hours() and not is_bedroom_safe():
        ...
        log_decision('arrival', [
            ...
            {'action': 'thermostat_comfort', 'reason': f'Set to {comfort_temp}°F'},
        ], source='event_bus',
        why=f'Silent arrival at {datetime.now().strftime("%H:%M")} — silent hours, alarm disarmed, thermostat {comfort_temp}°F')
        return
    decisions = decide(ctx)
    results   = execute_decisions(decisions)
    results.append({'action': 'alarm_disarm', 'reason': 'Arrival — auto-disarm'})
    results.append({'action': 'thermostat_comfort', 'reason': f'Set to {comfort_temp}°F'})
    update_mode()
...
def depart_sequence():
    ...
    # v5.1: thermostat → eco mode on departure
    try:
        ha_post('/services/climate/set_preset_mode', {'entity_id': 'climate.home', 'preset_mode': 'eco'})
        decisions.append({'action': 'thermostat_eco', 'reason': 'Departure — eco mode saves energy'})
        logger.info('🌡️ Thermostat → eco mode (departed)')
    except Exception as e:
        decisions.append({'action': 'thermostat_eco_failed', 'reason': str(e)})
...
def mode_updater_thread():
    while True:
        ...
        sonos_follow_sweep()
        check_thermostat_anomaly()         # v5.1 Tier 4+5
        check_weather_reactive_thermostat() # v5.1 Tier 3
        time.sleep(60)
